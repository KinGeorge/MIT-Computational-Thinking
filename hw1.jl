...
# ╔═╡ ff205887-114a-46a9-96dd-580279033f9b
function get_greens(image::AbstractMatrix)
	greens = get_green.(image)
	return greens
end

# ╔═╡ 9eacc8a2-8b4d-40b1-8582-921791345297
function get_blue(pixel::AbstractRGB)
	blue = pixel.b
	return blue
end

# ╔═╡ d2c9ff73-e692-42eb-a75c-d982e49a99f8
...
function mean_color(image)
	...
	mean_red = mean(mean.(reds))
	mean_green = mean(mean.(greens))
	mean_blue = mean(mean.(blues))
	print(mean_red)
...
function invert(color::AbstractRGB)
	inverted = RGB(1-color.r, 1-color.g, 1-color.b)
	return inverted
end
...
function noisify(x::Number, s)
	r = 2 * rand() * s - s
	return clamp(x + r, 0, 1)
end

# ╔═╡ bd047701-d89e-4151-bc8f-504b40692c29
rand(-0.2:0.2)
...
function noisify(color::AbstractRGB, s)
	nr = noisify(color.r, s)
	ng = noisify(color.g, s)
	nb = noisify(color.b, s)
	return RGB(nr, ng, nb)
end
...
noisify(0.5, 0.2) # edit this test case!
