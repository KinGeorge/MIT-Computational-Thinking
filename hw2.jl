...
begin
	md"""
	$(@bind l Slider(0:20, default=0, show_value=true))
	"""
end
...
function box_blur_kernel(l)
	return [1 / (2*l+1) for i in 1:2*l+1]
end
...
function box_blur(v::AbstractArray, l)
	ex_v = [extend(v, i) for i in -l+1:l+length(v)]
	res = [mean(ex_v[i:i+2*l]) for i in 1:length(v)]
	return res
end
...
function convolve(v::AbstractVector, k)
	l = length(k) ÷ 2
	ex_v = [extend(v, i) for i in -l+1:l+length(v)]
	res = [sum(ex_v[i:i+2*l].* k) for i in 1:length(v)]
	return res
end
...
function convolve(M::AbstractMatrix, K::AbstractMatrix)
	...
	ex_row, ex_col = length(K_rows) ÷ 2, length(K_columns) ÷ 2
	ex_M = [extend(M, i, j) for
		i in -ex_row+1:M_rows+ex_row,
		j in -ex_col+1:M_columns+ex_col]
	
	return [sum(ex_M[i:i+2*ex_row, j:j+2*ex_col].* K) for 
				i in 1:M_rows,
				j in 1:M_columns]
end
...
function gaussian_kernel_1D(n; σ = 1)
	sampled = [gauss(i, σ) for i in -n:n]
	return sampled / sum(sampled)
end
...
function with_gaussian_blur(image; σ=3, l=5)
	kernel = [gauss(x, y ; σ) for
				x in -l:l,
				y in -l:l]
	normed_kernel = kernel / sum(kernel)
